...
public class JavaString2 {
	...
	public static void main(String[] args) {
		...
		word=" "; // isEmpty는 띄어쓰기도 문자가 존재하는걸로 인지하고 isBlank는 비어있는걸로 간주한다.
		System.out.println(word.isEmpty()); 
		System.out.println(word.isBlank());
		// empty는 문자열의 인덱스가 존재하지 않아야 true이고 Blank는 띄어쓰기와 같이 빈공간인 경우에 비어있다로 표시 true
		System.out.println(word.length());
		
//		word=null; //null이 저장되어 있는 경우에는 문자열 관련 메소드 사용불가
//		System.out.println(word.isEmpty());
//		System.out.println(word.isBlank());
		...
		/* 정규 표현식
		   . 임의의 문자 1개를 의미
		   ^ 시작을 의미(^a : a로 시작하는 단어)
		   $ $앞의 문자열로 끝나는지를 의미(a$ : a로 끝나는 단어)
		   [] []괄호 안의 문자가 있는지 확인해라 뜻([^a]:a로 시작하는 단어)
